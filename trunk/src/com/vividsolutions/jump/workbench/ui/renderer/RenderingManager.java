...
public class RenderingManager {
	 ...
	 */
	public static final String USE_MULTI_RENDERING_THREAD_QUEUE_KEY = RenderingManager.class
			.getName()
	 ...
	 * queries to be done in parallel. But not too many, as each Thread consumes
	 * 1 MB of memory (see http://mindprod.com/jglossthread.html). The Threads
	 * may pile up if the server is down. [Jon Aquino]
	 */
	...
	protected List contentIDs() {
		...
		for (Iterator i = panel.getLayerManager().reverseIterator(
				Layerable.class); i.hasNext();) {
	...
	protected Renderer createRenderer(Object contentID) {
		...
		if (contentIDToHighRendererFactoryMap.containsKey(contentID)) {
			return ((Renderer.Factory) contentIDToHighRendererFactoryMap
					.get(contentID)).create();
		}
