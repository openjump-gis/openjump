 ...
 *
 * Copyright (C) Micha�l Michaud (2017)
 *
...
public class OpenJUMPMakeValidOp extends MakeValidOp {
    ...
    //public OpenJUMPMakeValidOp setPreserveGeomDim(boolean preserveGeomDim) {
    //    this.preserveGeomDim = preserveGeomDim;
    //    return this;
    //}

    //public OpenJUMPMakeValidOp setPreserveCoordDim(boolean preserveCoordDim) {
    //    this.preserveCoordDim = preserveCoordDim;
    //    return this;
    //}

    //public OpenJUMPMakeValidOp setPreserveDuplicateCoord(boolean preserveDuplicateCoord) {
    ...
    public Geometry makeValid(Geometry geometry) {
        ...
        for (Geometry g : list2) {
            ...
            if (getPreserveGeomDim() && !geometry.getClass().getSimpleName().equals("GeometryCollection")) {
        ...
        if (list2.isEmpty()) {
            ...
            if (csFactory instanceof PackedCoordinateSequenceFactory
                    && coordDim == 4) {
    ...
    private Geometry makeLineStringValid(LineString lineString) {
        ...
        if (sequenceWithoutDuplicates.size() == 0) {
            ...
            if (getPreserveGeomDim()) {
                return factory.createLineString(factory.getCoordinateSequenceFactory().create(0, sequence.getDimension()));
            } else {
                return factory.createPoint(sequenceWithoutDuplicates);
            }
        } else if (getPreserveDuplicateCoord()){
    ...
    public static void main(String[] args) throws ParseException {
        ...
        WKTReader reader;
        OpenJUMPMakeValidOp op = new OpenJUMPMakeValidOp();
        MakeValidOp opGeomDimNotPreserved = new OpenJUMPMakeValidOp().setPreserveGeomDim(false);
        MakeValidOp opDupCoordNotPreserved = new OpenJUMPMakeValidOp().setPreserveDuplicateCoord(false);
        //MakeValidOp opCoordDimNotPreserved = new OpenJUMPMakeValidOp().setPreserveCoordDim(false);
        Geometry input, result;

        // -----------------------------------------------------
        // Test Point
        // -----------------------------------------------------
        Point p1 = factory3.createPoint(new Coordinate(0,0));
        Point p2 = op.makePointValid(p1);
        assert p1.equals(p2);

        // return an empty point if X or Y is NaN
        p1 = factory3.createPoint(new Coordinate(Double.NaN,0));
        p2 = op.makePointValid(p1);
        assert !p1.isEmpty();
        assert p2.isEmpty();

        // return an empty point if X or Y is NaN
        p1 = factory3.createPoint(new Coordinate(0, Double.NaN));
        p2 = op.makePointValid(p1);
        assert !p1.isEmpty();
        assert p2.isEmpty();

        // Preserve fourth coord dimension
        p1 = factory4.createPoint(DOUBLE_FACTORY.create(new double[]{0,1,2,3}, 4));
        p2 = op.makePointValid(p1);
        ...
        input = factory4.createPolygon(cs1);
