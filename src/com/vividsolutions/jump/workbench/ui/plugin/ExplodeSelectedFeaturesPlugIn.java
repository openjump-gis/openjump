...
public class ExplodeSelectedFeaturesPlugIn extends AbstractPlugIn {
  ...
  public boolean execute(final PlugInContext context) throws Exception {
    ...
    return EditTransaction.commit(transactions,
        new EditTransaction.SuccessAction() {
          public void run() {
            // SuccessActions don't run after redos, which is the behaviour we
            // want here. [Jon Aquino]
  ...
  private Collection newFeatures(EditTransaction transaction) {
    ...
    for (java.util.Iterator<Feature> i = transaction.getFeatures().iterator(); i
        .hasNext();) {
      Feature f = i.next();
  ...
  private List explode(Collection features) {
    ...
    for (Iterator i = features.iterator(); i.hasNext();) {
      ...
      GeometryCollection collection = (GeometryCollection) feature
          .getGeometry();
      // clone the feature and nullify its geometry before
      // feature = (Object)feature.clone();
      // feature.setGeometry(collection.getFactory().createGeometryCollection(new
      // Geometry[0]));
    ...
    return new MultiEnableCheck()
        ...
        .add(new EnableCheck() {
          public String check(JComponent component) {
            Collection featuresWithSelectedItems = workbenchContext
