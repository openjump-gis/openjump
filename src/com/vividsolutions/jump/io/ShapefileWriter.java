...
public class ShapefileWriter implements JUMPWriter {
  ...
  private void writeDbf(FeatureCollection featureCollection, String fname, Charset charset) throws Exception {
    ...
    // Loop through FeatureSchema to set DbfFieldDef[]
    for (t = 0; t < fs.getAttributeCount(); t++) {
      ...
      //[mmichaud 2012-10-07] change from 11 to 10 char (to conform to dbf specification)
    ...
    // Loop through features and transform attributes array into Vector<Object>
    // according to dbf specification
  ...
  private DbfFieldDef overrideWithExistingCompatibleDbfFieldDef(DbfFieldDef field, Map columnMap) {
    ...
    if ((columnMap != null) && (columnMap.containsKey(fieldname))) {
      ...
      switch (dbfFieldDef.fieldtype) {
        case 'C':
        case 'c':  //character case not working yet
  ...
  private int findMaxStringLength(FeatureCollection fc, int attributeIndex, Charset charset) {
    int maxlen = 1; //LDB: don't allow zero length strings
    for (Feature feature : fc.getFeatures()) {
      //patch from Hisaji Ono for Double byte characters
      if (feature.getAttribute(attributeIndex) == null) {
        continue;
      }
      int length = feature.getString(attributeIndex).getBytes(charset).length;
      if (length > maxlen) {
        maxlen = length;
      }
    }
  ...
  private void checkIfGeomsAreMixed(FeatureCollection featureCollection) throws Exception {
    ...
    for (Feature feature : featureCollection.getFeatures()) {
      ...
      if (i == 0) {
        ...
        if (firstClass != geom.getClass()) {
          // System.out.println("first test failed");
          if ((firstGeom instanceof Polygon) && (geom instanceof MultiPolygon)) {
            //everything is ok
          } else if ((firstGeom instanceof MultiPolygon) && (geom instanceof Polygon)) {
            //everything is ok
          } else if ((firstGeom instanceof Point) && (geom instanceof MultiPoint)) {
            //everything is ok
          } else if ((firstGeom instanceof MultiPoint) && (geom instanceof Point)) {
            //everything is ok
          } else if ((firstGeom instanceof LineString) && (geom instanceof MultiLineString)) {
            //everything is ok
          } else if ((firstGeom instanceof MultiLineString) && (geom instanceof LineString)) {
            //everything is ok
          } else {
            throw new IllegalParametersException(
