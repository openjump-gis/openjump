...
public class SaveDatasetsPlugIn extends AbstractPlugIn {
  ...
  public boolean execute(PlugInContext context) throws Exception {
    try {
      ...
      FileFilter fileFilter1 = GUIUtil.createFileFilter("SHP " + sFiles,
          new String[] { "shp" });
      fileChooser.addChoosableFileFilter(fileFilter1);
      FileFilter fileFilter2 = GUIUtil.createFileFilter("GML " + sFiles,
          new String[] { "gml" });
      fileChooser.addChoosableFileFilter(fileFilter2);
      FileFilter fileFilter3 = GUIUtil.createFileFilter("JML " + sFiles,
          new String[] { "jml" });
      fileChooser.addChoosableFileFilter(fileFilter3);
      FileFilter fileFilter4 = GUIUtil.createFileFilter("FME " + sFiles,
          new String[] { "fme" });
      fileChooser.addChoosableFileFilter(fileFilter4);
      FileFilter fileFilter5 = GUIUtil.createFileFilter("WKT " + sFiles,
          new String[] { "wkt" });
      ...
      if (saveAll) {
        layerCollection = layerManager
            .getLayersWithModifiedFeatureCollections();
        for (Iterator i = layerCollection.iterator(); i.hasNext();)
          layerList.add((Layer) i.next());
      } else {
        layerCollection = (Collection) context.getWorkbenchContext()
            .getLayerNamePanel().selectedNodes(Layer.class);
        for (Iterator i = layerCollection.iterator(); i.hasNext();) {
          ...
          if (dsq != null)
            if (!dsq.getDataSource().isWritable())
              addIt = true;
          if (layer.isFeatureCollectionModified() || addIt) // just add modified
                                                            // or read-only
                                                            // layers
            layerList.add(layer);
        }
      ...
      for (int i = layerList.size() - 1; i >= 0; i--) {
        ...
        if (layer.getFeatureCollectionWrapper().getFeatures().size() == 0) // layer
                                                                           // is
                                                                           // empty
        {
          context.getWorkbenchFrame().getOutputFrame()
              .addText(sEmptyLayerNotSaved + ": " + layer.getName());
          writeWarning = true;
          layerList.remove(i);
          newLine = "\n";
        }
      ...
      for (int i = layerList.size() - 1; i >= 0; i--) {
        ...
        if (writeSaveMsg) {
          context.getWorkbenchFrame().getOutputFrame()
              .addText(sUseSaveDatasetAsToSaveLayer + layer.getName());
          writeWarning = true;
          layerList.remove(i);
          newLine = "\n";
        }
      ...
      saveReadOnlySources = -1; // initialize so that we ask user if these are
                                // to be saved
      pathToSaveReadOnlySources = ""; // initialize to that WriteLayer will ask
                                      // the first time
        ...
        if (!dsq.getDataSource().isWritable()) // data source is read-only
        {
          if (saveReadOnlySources == -1) {
            int response = JOptionPane.showConfirmDialog(
                workbenchContext.getLayerViewPanel(), sWantToSaveReadonly
                    + "\n" + "(" + sNoteLayerNameWillBeFileName + ")", "JUMP",
                JOptionPane.YES_NO_OPTION);

            saveReadOnlySources = 0;
            if (response == JOptionPane.YES_OPTION) {
              if (JFileChooser.APPROVE_OPTION == fileChooser
                  .showSaveDialog(workbenchContext.getLayerViewPanel())) {
          ...
          if (saveReadOnlySources == 0) {
            context
                .getWorkbenchFrame()
                .getOutputFrame()
                .addText(newLine + sCanNotSaveReadOnly + ": " + layer.getName());
            writeWarning = true;
            layerList.remove(i);
          }
        }
      ...
      while (currRec < lastRec) {
        ...
        String currDestination = currLayer.getDataSourceQuery().getDataSource()
            .getProperties().get("File").toString();
        if (!currLayer.getDataSourceQuery().getDataSource().isWritable()) // read-only
                                                                          // source
          currDestination = pathToSaveReadOnlySources + currLayer.getName()
              + extToSaveReadOnlySources;
        ...
        while (checkRec <= lastRec) {
          ...
          String checkDestination = checkLayer.getDataSourceQuery()
              .getDataSource().getProperties().get("File").toString();
          if (!checkLayer.getDataSourceQuery().getDataSource().isWritable())
            checkDestination = pathToSaveReadOnlySources + checkLayer.getName()
                + extToSaveReadOnlySources;
          if (currDestination.equals(checkDestination)) // found duplicate
                                                        // source
          {
            dupLayers = dupLayers + "\n" + sLayerName + ": "
                + checkLayer.getName();
      ...
      // check to see if we need to warn user that files are about to be
      // replaced
      ...
      for (int i = 0; i < layerList.size(); i++) {
        ...
        if (!dsq.getDataSource().isWritable()) {
          destinationFile = pathToSaveReadOnlySources + layer.getName()
              + extToSaveReadOnlySources;

          if (new File(destinationFile).exists()) {
            numReplaced++;
            replacedFiles = replacedFiles + sReadOnlyLayer + ": "
                + layer.getName() + " " + sReplacesFile + ": "
                + destinationFile + "\n";
      ...
      if ((numReplaced > 1) || ((numReplaced == 1) && (!fileMatches))) // need
                                                                       // to ask
                                                                       // user
                                                                       // if it
                                                                       // is OK
                                                                       ...
                                                                       // files
      {
        ...
        int response = JOptionPane.showConfirmDialog(
            workbenchContext.getLayerViewPanel(), prompt, "JUMP",
            JOptionPane.OK_CANCEL_OPTION);

        if (response == JOptionPane.CANCEL_OPTION) {
          if (numReplaced == 1)
            context.getWorkbenchFrame().getOutputFrame()
                .addText(sWouldHaveReplaced + ":");
          else
            context.getWorkbenchFrame().getOutputFrame()
                .addText(sWouldHaveReplaced + ":");
      ...
      for (int i = 0; i < layerList.size(); i++) {
        ...
        if (writeLayer(context, layer)) {
          ...
          context.getWorkbenchFrame().getOutputFrame()
              .addText(sSavedLayer + ": " + layer.getName());
        } else {
          context.getWorkbenchFrame().getOutputFrame()
              .addText(sCouldNotSaveLayer + ": " + layer.getName());
        }
      ...
      if (saveAll) {
        if (context.getTask().getProjectFile() != null) {
          ...
          context
              ...
              .addText(
                  "\n " + sSavedTask + ": "
                      + context.getTask().getProjectFile().getName());
        }
      }

      if (writeWarning)
        context.getWorkbenchFrame().warnUser(
            sCouldNotSaveLayer + " --- " + sErrorSeeOutputWindow);
      ...
      context.getWorkbenchFrame().getOutputFrame()
          .addText("SaveDatasetsPlugIn Exception:" + e.toString());
      return false;
    }
  }
    ...
    return new MultiEnableCheck()
        ...
        .add(new EnableCheck() {
          public String check(JComponent component) {
            Layer[] lyrs = workbenchContext.getLayerNamePanel()
                .getSelectedLayers();
            boolean changesToSave = false;
            for (Layer lyr : lyrs) {
              if (!lyr.isReadonly() && lyr.hasReadableDataSource()
  ...
  private boolean writeLayer(PlugInContext context, Layer layer, String filename) {
    ...
    DataSourceQuery dsq = layer.getDataSourceQuery();
    /*
     * String filename = ""; if (dsq.getDataSource().isWritable()) { filename =
     * dsq.getDataSource().getProperties().get("File").toString(); } else
     * //read-only source { filename = pathToSaveReadOnlySources +
     * layer.getName() + extToSaveReadOnlySources;
     * 
     * //the following shouldn't happen if
     * (pathToSaveReadOnlySources.equals("")) {
     * context.getWorkbenchFrame().getOutputFrame().addText(sNoOutputDir + ": "
     * + layer.getName());
     * context.getWorkbenchFrame().warnUser(sWarningSeeOutputWindow); return
     * false; } if (extToSaveReadOnlySources.equals("")) {
     * context.getWorkbenchFrame().getOutputFrame().addText(sNoOutputFileExt +
     * ": " + layer.getName());
     * context.getWorkbenchFrame().warnUser(sWarningSeeOutputWindow); return
     * false; } }
     */
    ...
    try {
      if ((filename.toLowerCase()).endsWith(".shp")) {
        ...
        for (int i = 0; i < newLayers.size(); i++) {
          ...
          properties.put(DataSource.URI_KEY, new File(newFileName).toURI().toURL().toExternalForm());
          ...
          DataSourceQuery dataSourceQuery = new DataSourceQuery(dataSource,
              newLayer.getName(), null);
          newLayer.setDataSourceQuery(dataSourceQuery)
              .setFeatureCollectionModified(false);
          dp.set("File", newFileName);
          (new ShapefileWriter()).write(newLayer.getFeatureCollectionWrapper(),
              dp);
          context.getWorkbenchFrame().getOutputFrame()
              .addText(sSavedLayer + ": " + newLayer.getName());
        }
      ...
      List saveFileChoosers = DataSourceQueryChooserManager.get(
      ...
      for (Object chooser : saveFileChoosers) {
        if (chooser instanceof SaveFileDataSourceQueryChooser) {
          ...
          if (extension_matches) {
            dsq.getDataSource()
                /*
                 * saver.toDataSourceQuery(new File(filename)) .getDataSource()
                 */
                .getConnection()
                .executeUpdate(null, layer.getFeatureCollectionWrapper(),
      ...
      context
          ...
          .getOutputFrame()
          .addText(
              sUnrecognizedFileType + " --- " + sCouldNotSaveLayer + ": "
                  + layer.getName());
      context.getWorkbenchFrame().warnUser(sErrorSeeOutputWindow);
      return false;
    } catch (Exception e) {
      ...
      context.getWorkbenchFrame().getOutputFrame().createNewDocument();
      context.getWorkbenchFrame().getOutputFrame()
          .addText("SaveDatasetsPlugIn:WriteLayer Exception:" + e.toString());
      context.getWorkbenchFrame().getOutputFrame()
          .addText(Arrays.toString(e.getStackTrace()));
      return false;
    }
  }
  ...
  private boolean writeLayer(PlugInContext context, Layer layer,
    ...
    try {
      dataSource.getConnection().executeUpdate(null,
  ...
  private List splitLayer(PlugInContext context, Layer layer) {
    ...
    if (!CompatibleFeatures(layer)) {
      ...
      FeatureCollectionWrapper featureCollection = layer
          .getFeatureCollectionWrapper();
      List featureList = featureCollection.getFeatures();
      FeatureSchema featureSchema = layer.getFeatureCollectionWrapper()
          .getFeatureSchema();
      ...
      for (Iterator i = featureList.iterator(); i.hasNext();) {
        ...
        if (geometry.isEmpty()) // going to delete it
          emptyFeatures.add(feature);
            ...
            && (!(geometry instanceof MultiPolygon))) // mixed geometry; going
                                                      // to explode it
      ...
      for (int i = 0; i < groupFeatures.size(); i++) // delete
                                                     // GeometryCollections
      {
        Feature feature = (Feature) groupFeatures.get(i);
        GeometryCollection geometry = (GeometryCollection) feature
            .getGeometry();
        explodeGeometryCollection(featureSchema, pointFeatures, lineFeatures,
            polyFeatures, geometry, feature);
      ...
      // now add just the exploded features that belong on the the original
      // layer
      ...
      if (pointFeatures.size() > 0) {
        Layer pointLayer = context.addLayer(
            ...
            layer.getName() + "_point", new FeatureDataset(featureSchema));

        FeatureCollectionWrapper pointFeatureCollection = pointLayer
            .getFeatureCollectionWrapper();
        newLayers.add(pointLayer);
        context.getWorkbenchFrame().getOutputFrame()
            .addText(sNewLayerCreated + ": " + pointLayer.getName());
        context.getWorkbenchFrame().warnUser(
            sNewLayerCreated + " - " + sWarningSeeOutputWindow);
      ...
      if (lineFeatures.size() > 0) {
        Layer lineLayer = context.addLayer(
            selectedCategories.isEmpty() ? StandardCategoryNames.WORKING
                : selectedCategories.iterator().next().toString(),
            layer.getName() + "_line", new FeatureDataset(featureSchema));

        FeatureCollectionWrapper lineFeatureCollection = lineLayer
            .getFeatureCollectionWrapper();
        newLayers.add(lineLayer);
        context.getWorkbenchFrame().getOutputFrame()
            .addText(sNewLayerCreated + ": " + lineLayer.getName());
        context.getWorkbenchFrame().warnUser(
            sNewLayerCreated + " - " + sErrorSeeOutputWindow);
      ...
      if (polyFeatures.size() > 0) {
        Layer polyLayer = context.addLayer(
            selectedCategories.isEmpty() ? StandardCategoryNames.WORKING
                : selectedCategories.iterator().next().toString(),
            layer.getName() + "_area", new FeatureDataset(featureSchema));

        FeatureCollectionWrapper polyFeatureCollection = polyLayer
            .getFeatureCollectionWrapper();
        newLayers.add(polyLayer);
        context.getWorkbenchFrame().getOutputFrame()
            .addText(sNewLayerCreated + ": " + polyLayer.getName());
        context.getWorkbenchFrame().warnUser(
            sNewLayerCreated + " - " + sErrorSeeOutputWindow);
  ...
  private void explodeGeometryCollection(FeatureSchema fs,
      ArrayList pointFeatures, ArrayList lineFeatures, ArrayList polyFeatures,
      GeometryCollection geometryCollection, Feature feature) {
    for (int i = 0; i < geometryCollection.getNumGeometries(); i++) {
      Geometry geometry = geometryCollection.getGeometryN(i);

      if (geometry instanceof GeometryCollection) {
        explodeGeometryCollection(fs, pointFeatures, lineFeatures,
        ...
        if (featureBit.get(GeoUtils.pointBit))
          pointFeatures.add(newFeature);
        if (featureBit.get(GeoUtils.lineBit))
          lineFeatures.add(newFeature);
        if (featureBit.get(GeoUtils.polyBit))
          polyFeatures.add(newFeature);
      }
    }
  }

  private void writeProjectionFile(PlugInContext context, Layer outputLayer)
      throws IOException { // per LDB projection files only associated with .shp
                           // files; confirmed 8/16/05
    ...
    if (dsqOut != null
      ...
      String outputFileName = dsqOut.getDataSource().getProperties()
          .get("File").toString();
      if ((outputFileName.toLowerCase()).endsWith(".shp")) {
        ...
        if (!(new File(outputPrjFileName).exists())) { // loop through all
                                                       // layers to find a
                                                       // project file; then
                                                       // copy contents
          ...
          for (Iterator i = layerList.iterator(); i.hasNext();) {
            ...
            DataSourceQuery dsq = layer.getDataSourceQuery();
            if (dsq != null
                && dsq.getDataSource().getProperties().get("File") != null) {
              String inputFileName = dsq.getDataSource().getProperties()
                  .get("File").toString();
                ...
                if (new File(inputPrjFileName).exists()) {
                  ...
                  try {
                    ...
                    context
                        ...
                        .getOutputFrame()
                        .addText(
                            sCouldNotWrite + ": " + outputPrjFileName + " --- "
                                + ex.getMessage());
                  }
                  break;
                }
