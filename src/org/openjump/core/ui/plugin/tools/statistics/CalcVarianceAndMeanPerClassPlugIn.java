...
public class CalcVarianceAndMeanPerClassPlugIn extends AbstractPlugIn implements ThreadedPlugIn {
   ...
   * It is called before the "run" method and useful to do all the GUI /user-input
   * things
   * In this example we call two additional methods
   * {@link #setDialogValues(MultiInputDialog, PlugInContext)}
   * and {@link #getDialogValues(MultiInputDialog)} to obtain the Layer and the
  ...
  public boolean execute(PlugInContext context) throws Exception {
    ...
    MultiInputDialog dialog = new MultiInputDialog(context.getWorkbenchFrame(), sName, true);
  ...
  private boolean calcClass(final PlugInContext context, TaskMonitor monitor, Layer orgLayer) throws Exception {
    ...
    for (Map.Entry<Object, List<Double>> entry : classes.entrySet()) {
      ...
      mean_var.put(entry.getKey(),
          new double[] { (vals == null || vals.length == 0) ? Double.NaN : StatisticSample.mean(vals),
