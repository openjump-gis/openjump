...
public class IntersectPolygonLayersPlugIn extends ThreadedBasePlugIn {
	  ...
	  public void run(TaskMonitor monitor, PlugInContext context) throws Exception{
	   ...
	   /*
	    FeatureCollection resultColl = runSpatialJoinMethod(layer1.getFeatureCollectionWrapper(),
	        ...
	        methodNameToRun);
	    */
	    FeatureCollection resultColl = runIntersection(layer1.getFeatureCollectionWrapper(), layer2.getFeatureCollectionWrapper(), monitor);
	  if (resultColl.size()>0)
	    context.addLayer(StandardCategoryNames.WORKING, I18N.get("org.openjump.sigle.plugin.SpatialJoinPlugIn.Result"), resultColl);
	    if (exceptionThrown)
	  ...
	  private FeatureCollection runIntersection(FeatureCollection fcA, FeatureCollection fcB, TaskMonitor monitor){
		  ...
		  // TODO: sort out the different geometry types
		  ArrayList<Geometry> withoutIntersection = IntersectGeometries.intersectGeometries(geomsToCheck, monitor, context);
...
//    	    	}
	  ...
	  private void setDialogValues(MultiInputDialog dialog, PlugInContext context)
	  {
	    ...
	    dialog.setSideBarDescription("Intersects all geometries of two layers that contain either both polygons or linestrings");
