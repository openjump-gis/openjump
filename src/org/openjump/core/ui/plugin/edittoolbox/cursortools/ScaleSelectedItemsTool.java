...
public class ScaleSelectedItemsTool extends DragTool implements ShortcutsDescriptor {
  ...
  public ScaleSelectedItemsTool(WorkbenchContext context) {
    super(context);
    this.scaleSelectedItems = I18N.getInstance()
        .get("org.openjump.core.ui.plugin.edittoolbox.cursortools.ScaleSelectedItemsTool.Scale-Selected-Items");
    this.sScaleFactor = I18N.getInstance()
        .get("org.openjump.core.ui.plugin.edittoolbox.cursortools.ScaleSelectedItemsTool.scale-factor");

    this.checkFactory = EnableCheckFactory.getInstance(context);

    this.originalStroke = new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[] { 3, 3 }, 0);
  ...
  protected void gestureFinished() throws java.lang.Exception {
    ...
    if (this.startScaling == true) {
      ...
      /*
       * //-- visualize box Polygon box = (Polygon)this.originalBBox.clone();
       * //cloned! PolygonScale.scalePolygon(box,xscale, yscale,center.x,center.y);
       * ArrayList geoms = new ArrayList(); geoms.add(box); FeatureCollection myFC =
       * FeatureDatasetFactory.createFromGeometry(geoms);
       * getPanel().getLayerManager().addLayer(StandardCategoryNames.WORKING, "fbox",
       * myFC);
       */
      this.startScaling = false;
   ...
   * EditTransaction.SelectionEditor() { public Geometry edit(Geometry
   * geometryWithSelectedItems, Collection selectedItems) { for (Iterator j =
   * selectedItems.iterator(); j.hasNext();) { Geometry item = (Geometry)
   * j.next(); if(item instanceof Polygon){
   * PolygonScale.scalePolygon((Polygon)item,xscale, yscale, center.x,center.y); }
   * else{ getPanel().getContext().warnUser("no polygon!"); } } return
   * geometryWithSelectedItems; } }, getPanel(), getPanel().getContext(),
   * getName(), layer, isRollingBackInvalidEdits(), false); return transaction; }
   **/
  ...
  public void mousePressed(MouseEvent e) {
    ...
    double tolerance = SnapManager.getToleranceInPixels(this.getWorkbench().getBlackboard())
        / this.getPanel().getViewport().getScale();
    // -- calc a buffer around the corner points.. so that only near to corner
    // points scaling is activated
    ...
    /*
     * //-- visualize ArrayList geoms = new ArrayList(); geoms.add(buffergeom);
    ...
    if (buffergeom.contains(mousep)) {
      try {
        ...
        // -- test: use this if drawing the bounding box only (=> change therfore
        // #getShape() as well)
        // this.selectionBBoxShape =
        // getPanel().getJava2DConverter().toShape(this.originalBBox);
        ...
        /*
         * //-- visualize ArrayList geoms = new ArrayList(); geoms.add(new
         * GeometryFactory().createPoint(this.center)); FeatureCollection myFC =
         * FeatureDatasetFactory.createFromGeometry(geoms);
         * getPanel().getLayerManager().addLayer(StandardCategoryNames.WORKING,
         * "CenterSet", myFC);
         */
    ...
    if (startScaling == true) {
      ...
      try {
        ...
        getPanel().getContext().setStatusMessage(
            sScaleFactor + " x: " + df2.format(xscale) + "  " + sScaleFactor + " y: " + df2.format(yscale));
        /*
         * //-- reset shape of selectedFeatureShape = bbox Polygon box =
         * (Polygon)this.originalBBox.clone(); //cloned!
  ...
  /*
   * //overwrite method public Cursor getCursor() { return
  ...
  public void mouseMoved(MouseEvent e) {
    try {
      ...
      // Geometry buffergeom =
      // this.originalBBox.getBoundary().buffer(tolerance*this.toleranceFactor);
      // -- calc a buffer around the corner points.. so that only near to corner
      // points scaling is activated
      LineString ls = (LineString) this.originalBBox.getBoundary();
      MultiPoint mps = new GeometryFactory().createMultiPoint(ls.getCoordinates());
  ...
  private Geometry createSelectedItemsBoundingBox() {
    // private Shape createSelectedItemsBoundingBox() throws
    ...
    Coordinate[] coords = new Coordinate[] { new Coordinate(xmin, ymin), new Coordinate(xmin, ymax),
        new Coordinate(xmax, ymax), new Coordinate(xmax, ymin), new Coordinate(xmin, ymin) };
