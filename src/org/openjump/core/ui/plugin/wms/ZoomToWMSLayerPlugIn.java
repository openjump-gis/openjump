...
public class ZoomToWMSLayerPlugIn extends AbstractPlugIn {
  ...
  public void initialize(PlugInContext context) throws Exception {
    ...
    context.getFeatureInstaller().addMainMenuPlugin(this, new String[] { MenuNames.VIEW },
        I18N.getInstance().get("org.openjump.core.ui.plugin.wms.ZoomToWMSPlugIn.zoom-to-wms-layer") + "{pos:9}", false,
        null, enableCheck);
    // Add PlugIn to WMSPopupMenu
    context.getFeatureInstaller().addPopupMenuPlugin(context.getWorkbenchFrame().getWMSLayerNamePopupMenu(), this,
        I18N.getInstance().get("org.openjump.core.ui.plugin.wms.ZoomToWMSPlugIn.zoom-to-wms-layer"), false, null,
        enableCheck);
  ...
  public boolean execute(PlugInContext context) throws Exception {
    ...
    // String srs = null;
    // for (Object o :
    // context.getLayerableNamePanel().selectedNodes(WMSLayer.class)) {
    ...
    Map<String, BoundingBox> boundingBoxesForSRS = getBoundingBoxesForSRS(mapLayerOfChoosenLayers, selectedSRS);
    zoomToBoundingBox(context, boundingBoxesForSRS, selectedSRS);

    // Logger.info("Zoom to " +
    // context.getLayerableNamePanel().selectedNodes(WMSLayer.class) + " : " +
    // envelope);
    // context.getLayerViewPanel().getViewport().zoom(EnvelopeUtil.bufferByFraction(envelope,
    // 0.03));
    return true;
  }
  ...
  private List<MapLayer> getMapLayerOfChoosenLayers(PlugInContext context) throws Exception {
    ...
    if (selectedWmsLayers.length == 0) {
      JOptionPane.showMessageDialog(context.getWorkbenchFrame(),
          I18N.getInstance().get("org.openjump.core.ui.plugin.wms.ZoomToWMSPlugIn.no-wms-layer-selected"));
  ...
  private void zoomToBoundingBox(PlugInContext context, Map<String, BoundingBox> boundingBoxesForSRS,
    ...
    int back = JOptionPane.showConfirmDialog(context.getWorkbenchFrame(), jp,
        I18N.getInstance().get("org.openjump.core.ui.plugin.wms.ZoomToWMSPlugIn.bounding-box-for") + " " + selectedSRS
            + tmpLatLon,
        JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE);

    if (back == JOptionPane.CANCEL_OPTION || back < 0)
      return;
  ...
  private JComboBox<String> makeComboBox(Map<String, BoundingBox> boundingBoxesForSRS) {
    ...
    if (boundingBoxesForSRS.size() > 0) {
      ...
      comboBox.addItem(
          I18N.getInstance().get("org.openjump.core.ui.plugin.wms.ZoomToWMSPlugIn.no-bounding-boxes-available"));
    }
    return comboBox;
  }
  ...
  void showInformationTable(PlugInContext context) throws Exception {
    ...
    ZoomToWMSLayerPlugIn.InfoTableModel itm = new ZoomToWMSLayerPlugIn.InfoTableModel();
  ...
  Object[][] getMapLayerInformationForTable(PlugInContext context) throws Exception {
    ...
    ZoomToWMSLayerPlugIn.MapLayerAttributes mapLayerAttr = new ZoomToWMSLayerPlugIn.MapLayerAttributes();
  ...
  ZoomToWMSLayerPlugIn.MapLayerAttributes[] toMapLayerAttributesArray(Object[][] m) {
    ...
    for (int i = 0; i < numRows; i++) {
      ...
      mapLayerAttributesArray[i] = new ZoomToWMSLayerPlugIn.MapLayerAttributes(title, name, srs, minx, miny, maxx,
          maxy);
    }
  ...
  private static class MapLayerAttributes implements Comparable {
    ...
    static int[] sortUpDown = { SORT_DOWN, SORT_DOWN, SORT_DOWN, SORT_DOWN, SORT_DOWN, SORT_DOWN, SORT_DOWN };
    ...
    List<MapLayerAttributes> getMapLayerRows(MapLayer mapLayer) {
      ...
      for (BoundingBox bb : boundingBoxList) {
        ...
        mapLayerRows.add(new ZoomToWMSLayerPlugIn.MapLayerAttributes(title, name, srs, minX, minY, maxX, maxY));
      }
