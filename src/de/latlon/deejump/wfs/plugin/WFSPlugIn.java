...
public class WFSPlugIn extends ThreadedBasePlugIn {
	...
	public void initialize(PlugInContext context) throws Exception {
		 ...
		 * 2007-05-08 context.getFeatureInstaller().addMainMenuItem(this, new
		 * String[] { MenuNames.LAYER }, getName(), false, getIcon(), mec);
		 */
	}
	...
	public void run(TaskMonitor monitor, PlugInContext context)
		...
		if (monitor instanceof TaskMonitorV2)
			((TaskMonitorV2) monitor)
					.setTitle(i18n("read-response-to-tempfile"));
		InputStream is = WFSClientHelper.createResponseStreamfromWFS(panel
				.getWfService().getGetFeatureURL(), request);
		File tmpFile = FileUtil.copyInputStreamToTempFile(is, "wfs", null,
				monitor);
		...
		QualifiedName[] geoms = wfs
				.getGeometryProperties(ftName.getLocalName());
	 ...
	 * @throws Exception
	 *             if something went wrong when building or wrapping the
	 *             geometries
	 */
	private org.deegree.model.spatialschema.Geometry getSelectedGeoAsGML(
		...
		if (geo instanceof GeometryCollection) {
			throw new WorkbenchException(
					i18n("invalid-geometry-type-geometrycollection"));
		}
