...
public class WorkbenchFrame extends JFrame implements LayerViewPanelContext,
	...
	JMenu fileMenu = (JMenu) FeatureInstaller.installMnemonic(
			new JMenu(MenuNames.FILE), menuBar);
	...
	private JDesktopPane desktopPane = new JDesktopPane() {
        {
            // Simple workaround for the following JUMP bug: if you maximize one JInternalFrame, then all
            // JInternalFrames get maximized (including attribute windows, undesirably). The workaround is
            // to use the DefaultDesktopManager instead of the one installed by the Windows L&F
            // (the WindowsDesktopManager). (Uwe Dalluege noticed that the problem occurred with the
            // Windows L&F but not the Metal L&F -- this observation led me to the solution).
			...
			addPopupMenuListener(new PopupMenuListener() {
				public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
					...
					setTitle((panel.selectedNodes(Layer.class).size() != 1) ? ("("
							+ panel.selectedNodes(Layer.class).size() + " "+I18N.get("ui.WorkbenchFrame.layers-selected")+")")
			...
			addPopupMenuListener(new PopupMenuListener() {
				public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
					...
					setTitle((panel.selectedNodes(WMSLayer.class).size() != 1) ? ("("
							+ panel.selectedNodes(WMSLayer.class).size() + " "+I18N.get("ui.WorkbenchFrame.wms-layers-selected")+")")
	...
	private boolean confirmClose(String action, Collection modifiedLayers) {
		...
		JOptionPane pane = new JOptionPane(StringUtil.split(modifiedLayers
				...
				+ " "
				+ ((modifiedLayers.size() > 1) ? 
						I18N.get("ui.WorkbenchFrame.have-been-modified") 
